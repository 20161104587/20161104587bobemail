...
function reduction(emailid,userid,type,obj){
	//var b=document.getElementById('ti').value
	var del=window.confirm("您确定要恢复吗？");
	if(del){
	$.ajax({
	type:"POST",
	url:'${pageContext.request.contextPath}/emailController/reduction.do',
...
<div class="am-cf admin-main">
  ...
  <div class="am-cf am-padding">
    <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">垃圾箱</strong> / <small>还原</small></div>
  </div>

  <div class="am-tabs am-margin" data-am-tabs>
    <ul class="am-tabs-nav am-nav am-nav-tabs">
      <li class="am-active"><a href="#tab1">发件箱</a></li>
    ...
    <div class="am-tabs-bd">
      ...
      <div class="am-u-sm-12">
          ...
          <tbody>
         <!--  foreach循环 -->
         	<c:forEach items="${dustbinOutbox }" var="inlist" >
            <tr>
              <td><a href="#">${inlist.recvemail}</a></td>
             <td><a href="${pageContext.request.contextPath}/userController/checkemail.do?emailid=${inlist.mailid}" target="_Self">${inlist.title}</a></td>
             ...
             <td>
              <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="reduction(${inlist.mailid},${User.id},'outbox',this);">恢复</button>
              </td>
            </tr>
            </c:forEach>
          </tbody>
  ...
  <div class="am-fr">
    <ul class="am-pagination">
      ...
      <div class="am-u-sm-12">
          ...
          <tbody>
         <!--  foreach循环 -->
         	<c:forEach items="${dustbinInbox }" var="inlist" >
            <tr>
              <td><a href="#">${inlist.recvemail}</a></td>
             <td><a href="${pageContext.request.contextPath}/userController/checkemail.do?emailid=${inlist.mailid}" target="_Self">${inlist.title}</a></td>
              <td><a href="${pageContext.request.contextPath}/userController/download.do?filename=${inlist.attname}" target="_Self">${inlist.attname}</a></td>
              <td>${inlist.sendtime}</td>
             <td>
              <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="reduction(${inlist.mailid},${User.id},'inbox',this);">恢复</button>
              </td>
            </tr>
            </c:forEach>
          </tbody>
