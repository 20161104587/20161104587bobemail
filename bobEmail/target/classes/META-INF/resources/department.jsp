...
function del(depid,obj){
	var del=window.confirm("您确定要删除吗？");
	if(del){
	$.ajax({
	type:"POST",
	url:'${pageContext.request.contextPath}/userController/deletedepartment.do',
	data:{'depid':depid},
	dataType:'text',
	success:function(){
		  alert("成功拉");
      ...
      <div class="am-u-sm-12">
        <form class="am-form">
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-type">部门名称</th><th class="table-title">部门描述</th><th class="table-type">部门角色</th><th class="table-set">操作</th>
              </tr>
          </thead>
          <tbody>
         
         	<c:forEach items="${checkdepartment }" var="inlist" >
            <tr>
              <td><a href="#">${inlist.depname}</a></td>
              <td>${inlist.depdes}</td>
              <td>${inlist.deprole}</td>
             <td>
     <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="window.location.href = '${pageContext.request.contextPath}/userController/updatdepartment.do?depid=${inlist.depid}'"><span class="am-icon-trash-o"></span> 修改</button>
     <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="del(${inlist.depid},this);"><span class="am-icon-trash-o"></span> 删除</button>
